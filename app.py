...
# Set the environment variable for Clarifai
...
def about():
    ...
    with st.expander("Description"):
        st.write("""This app functionality includes 

1-Image Enhancement : 
Underwater images are vital in marine research, aquatic habitat inspection, and more. Light absorption and scattering cause color distortion, low contrast, and reduced visibility. To enhance image quality, zero-shot learning adapts related domain knowledge, compensating for color loss. The Laplacian pyramid isolates important features while reducing noise, improving visibility. Principal component analysis (PCA) reduces dimensionality, attenuating color distortions, and enhancing contrast. This integration of techniques enables underwater image enhancement, preserving valuable information for various applications like marine research and underwater surveillance
...
def classify_image(file_bytes, pat, user_id, app_id, model_id, model_version_id):
    ...
    return post_model_outputs_response.outputs[0].data.concepts
...
def coral_health_prediction():
    st.title("Coral Health Prediction App")
    ...
    if uploaded_file is not None:
        ...
        pat = clarifai_pat
        user_id = 'rehan22'
        app_id = 'coral_health_prediction_model'
        model_id = 'transfer_learning'
        model_version_id = '1c8d4d89f98c453a85a528c968dfc172'

        try:
            file_bytes = uploaded_file.read()
            classifier_output = classify_image(file_bytes, pat, user_id, app_id, model_id, model_version_id)

            st.write("Image Classifier predicted concepts:")
            for concept in classifier_output:
                st.write(f"{concept.name}: {concept.value:.2f}")

            manual_prompt = "define the health condition of coral reef in image?"
            prompt = f"{manual_prompt} - Image classifier output: {', '.join([f'{concept.name} ({concept.value:.2f})' for concept in classifier_output])}"
            ...
            gpt4_vision_model = Model("https://clarifai.com/openai/chat-completion/models/openai-gpt-4-vision")
            gpt4_vision_prediction = gpt4_vision_model.predict(inputs, inference_params=inference_params)

            st.write("\nGPT-4 Vision predicted text:")
            st.write(gpt4_vision_prediction.outputs[0].data.text.raw)
...
def underwater_image_enhancement(image, flag):
    ...
    st.text("White Balancing the compensated Image using Laplacian Pyramid...")
    whitebalanced=gray_world(compensatedimage)
    # Perform contrast enhancement using global Histogram Equalization
    st.text("Enhancing Contrast of White Balanced Image using Global Histogram Equalization and Zero-Shot Learning ...")
...
def main():
    selected_box = st.sidebar.selectbox('Select from dropdown', ( 'About the App','Underwater Image Enhancer', 'Coral Health Prediction'))
    ...
    elif selected_box == 'Coral Health Prediction':
        coral_health_prediction()
